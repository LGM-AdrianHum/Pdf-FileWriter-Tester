...
public class Barcode
	...
	public Int32[] CodeArray
		...
		get
			{
			return((Int32[]) _CodeArray.Clone());
			}
		}
	internal Int32[] _CodeArray;
	...
	/// </summary>
	public Int32 BarCount {get; protected set;}

	/// <summary>
	/// Total barcode width in narrow bar units.
	/// </summary>
	public Int32 TotalWidth {get; protected set;}
...
public class Barcode128 : Barcode
	...
	/// </summary>
	public const Int32	CODEA = 101;

	/// <summary>
	/// Special code CODEB (or FN4 for code set B).
	/// </summary>
	public const Int32	CODEB = 100;
	...
	public Barcode128
		...
		if(LeadDigitsEnd == TrailDigitsStart && LeadDigitsCount != 0)
			{
			// create code array
			_CodeArray = new Int32[1 + LeadFnc1End + (LeadDigitsEnd - LeadFnc1End) / 2 + (TextLen - TrailFnc1Start) + 2];
			...
			for(Int32 Index = 0; Index < LeadFnc1End; Index++) _CodeArray[CodeEnd++] = FNC1;

			// convert to pairs of digits
			EncodeDigits(LeadFnc1End, LeadDigitsEnd, ref CodeEnd);

			// add FNC1 if required
			for(Int32 Index = TrailFnc1Start; Index < TextLen; Index++) _CodeArray[CodeEnd++] = FNC1;
			}
		...
		// text has digits and non digits
			...
			_CodeArray = new Int32[2 * TextLen + 2];
			...
			Array.Resize<Int32>(ref _CodeArray, CodeEnd + 2);
			}
	...
	public Barcode128
		...
		// code array length
		Int32 Length = _CodeArray.Length;
		...
		if(_CodeArray[Length - 1] != STOP)
			...
			Length += 2;
			Array.Resize<Int32>(ref _CodeArray, Length);
			}
	...
	private void EncodeDigits
		...
		{
		// convert to pairs of digits
		for(Int32 Index = TextStart; Index < TextEnd; Index += 2) _CodeArray[CodeEnd++] = 10 * (Text[Index] - '0') + (Text[Index + 1] - '0');
		return;
		}
	...
	private void Checksum()
		{
		// calculate checksum
		Int32 Length = _CodeArray.Length - 2;
		Int32 ChkSum = _CodeArray[0];
		for(Int32 Index = 1; Index < Length; Index++) ChkSum += Index * _CodeArray[Index];
...
public class Barcode39 : Barcode
	...
	public Barcode39
		...
		// barcode array
		_CodeArray = new Int32[Text.Length + 2];
	...
	public Barcode39
		...
		if(_CodeArray[0] != START_STOP_CODE)
			{
			Int32[] TempArray = new Int32[_CodeArray.Length + 1];
		...
		if(_CodeArray[_CodeArray.Length - 1] != START_STOP_CODE)
			{
			Array.Resize<Int32>(ref _CodeArray, _CodeArray.Length + 1);
		...
		StringBuilder Str = new StringBuilder();
		for(Int32 Index = 1; Index < _CodeArray.Length - 2; Index++)
			{
			Int32 Code = _CodeArray[Index];
...
public class BarcodeEAN13 : Barcode
	...
	public BarcodeEAN13
		...
		// barcode array
		_CodeArray = new Int32[BARCODE_LEN];
...
public class BarcodeInterleaved2of5 : Barcode
	...
	public override Int32 BarWidth
		...
		// code index
		BarIndex -= 4;
		Int32 CodeIndex = 2 * (BarIndex / 10);
		if((BarIndex & 1) != 0) CodeIndex++;

		// code
		Int32 Code = _CodeArray[CodeIndex];
	...
	public BarcodeInterleaved2of5
		...
		foreach(Char Chr in Text)
			...
			_CodeArray[CodePtr++] = (Int32) (Chr - '0');
			}
	...
	private void Checksum()
		{
		// calculate checksum
		Int32 ChkSum = 3 * _CodeArray[0];
		Int32 End = _CodeArray.Length - 1;
		for(Int32 Index = 1; Index < End; Index += 2) ChkSum += _CodeArray[Index] + 3 * _CodeArray[Index + 1];

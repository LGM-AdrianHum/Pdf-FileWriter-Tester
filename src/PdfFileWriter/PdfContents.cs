...
public class PdfContents : PdfObject
	...
	public void SetColorNonStroking
		...
		{
		ObjectValueFormat("{0} {1} {2} rg\n", Round((Double) Color.R / 255.0), Round((Double) Color.G / 255.0), Round((Double) Color.B / 255.0));
		return;
		}
	...
	public void SetAlphaStroking
		...
		{
		SetAlphaStroking((Double) Color.A / 255.0);
		return;
		}
	...
	public void SetAlphaNonStroking
		...
		{
		SetAlphaNonStroking((Double) Color.A / 255.0);
		return;
		}
	...
	internal Double DrawTextInternal
		...
		Boolean GlyphIndexFlag = false;

		// text width
		Int32 Width = 0;

		// loop for all text characters
		for(Int32 Ptr = 0; Ptr < Text.Length; Ptr++)
			{
	...
	internal Double DrawTextWithKerning
		...
		for(;;)
			...
			Width += (Int32) Math.Round(KA.Adjust * Font.DesignHeight / 1000.0, 0, MidpointRounding.AwayFromZero);
			}
	...
	public void DrawQRCode
		...
		// draw QR Code
		ObjectValueFormat("q {0} 0 0 {0} {1} {2} cm {3} Do Q\n",

...
internal class QREncoder : QRCode
	...
	private void EncodeQRCode
		...
		OutputMatrix = new Boolean[MatrixDimension, MatrixDimension];

		// convert result matrix to output matrix
		for(Int32 Row = 0; Row < MatrixDimension; Row++)
			{
			for(Int32 Col = 0; Col < MatrixDimension; Col++) OutputMatrix[Row, Col] = (ResultMatrix[Row, Col] & 1) != 0;
			}
